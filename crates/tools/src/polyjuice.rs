...
pub fn polyjuice_call(
    ...
    let to_id = {
        ...
        let to_script_hash = godwoken_rpc_client.get_script_hash_by_registry_address(
            &RegistryAddress::new(ETH_REGISTRY_ACCOUNT_ID, to_address),
        )?;
        godwoken_rpc_client.get_account_id_by_script_hash(to_script_hash)?
    };
    let to_id = to_id.expect("to id not found!");

    let from_script_hash = parse_account_from_str(&mut godwoken_rpc_client, from)?;
...
fn send(
    ...
    let to_id = match to_address.clone() {
        ...
        Some(addr) => {
            let script_hash = godwoken_rpc_client.get_script_hash_by_registry_address(
                &RegistryAddress::new(ETH_REGISTRY_ACCOUNT_ID, addr),
            )?;
    ...
    let receiver_script_hash = godwoken_rpc_client.get_script_hash(to_id)?;
